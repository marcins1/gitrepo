...
int szukaj_bin_rek(int t[], int szuk, int p, int k){
    if(p <= k){
        int s = (p+k) / 2;
        if(t[s] == szuk)
            return s;
        if(szuk < t[s])
            return szukaj_bin_rek(t, szuk, p, s-1);
        else 
            return szukaj_bin_rek(t, szuk, s+1, k);
    }
    return -1;
}
...
int main(int argc, char **argv)
{
    ...
    //indeks = szukaj_bin_rek(tab, szuk, 0, n-1);
