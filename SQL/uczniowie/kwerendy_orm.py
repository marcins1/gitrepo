...
def kw02():
    query = (Uczen
        ...
        .join(Klasa).order_by(Uczen.nazwisko.asc())
    )
    for obj in query:
...
def kw06():
    ...
    query = (Klasa
        .select(Klasa.nazwa, fn.COUNT(Uczen.imie).alias('ilosc_uczniow'))
        .join(Uczen)
        .group_by(Klasa.nazwa)
        .order_by(Klasa.nazwa.asc())
    )
    for obj in query:
        print(obj.nazwa, obj.ilosc_uczniow)
